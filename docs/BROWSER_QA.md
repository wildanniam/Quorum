# Quorum Browser QA

> Historical local browser snapshot. It predates the final submission-recovery
> branch and does not verify the current Vercel deployment. Regenerate it only
> through the approved browser workflow and an isolated writable database.

Generated at: `2026-07-12T07:29:02.848Z`

This file is generated by `npm run browser:qa`. It records local browser
verification for the hackathon demo surface and complements
`docs/DEMO_EVIDENCE.md`, which records command-based verification.

## Environment

- App URL: `http://127.0.0.1:3040`
- Database setup: `npm run db:migrate` and `npm run db:seed`
- Seeded paid event: `APAC Stellar Builder Meetup`
- Seeded free event: `Stellar Open Office Hours`

## Summary

- Pages checked per viewport: `13`
- Viewports checked: `1440 x 900, 1024 x 768, 390 x 844`
- Console errors: none observed
- Horizontal overflow: none observed
- Missing required text: none observed

## Wide desktop

- Viewport: `1440 x 900`

| Page | Path | HTTP | Required text | Horizontal overflow | Console/page errors |
|---|---|---:|---|---|---:|
| Landing | `/` | 200 | yes | no | 0 |
| Discover | `/discover` | 200 | yes | no | 0 |
| Paid event detail | `/events/apac-stellar-builder-meetup` | 200 | yes | no | 0 |
| Checkout | `/events/apac-stellar-builder-meetup/checkout` | 200 | yes | no | 0 |
| Locked resources | `/events/apac-stellar-builder-meetup/resources` | 200 | yes | no | 0 |
| Pass library | `/passes` | 200 | yes | no | 0 |
| Pass receipt | `/passes/qpass-apac-stellar-builder-meetup-0001-0dc175` | 200 | yes | no | 0 |
| Organizer check-in | `/check-in/evt_apac_stellar_builder_meetup?token=qpass-apac-stellar-builder-meetup-0001-0dc175` | 200 | yes | no | 0 |
| Event proof | `/events/apac-stellar-builder-meetup/proof` | 200 | yes | no | 0 |
| Studio wallet gate | `/dashboard` | 200 | yes | no | 0 |
| Create event | `/dashboard/events/new` | 200 | yes | no | 0 |
| Collaborator ledger | `/dashboard/ledger` | 200 | yes | no | 0 |
| Evidence hub | `/evidence` | 200 | yes | no | 0 |

## Tablet

- Viewport: `1024 x 768`

| Page | Path | HTTP | Required text | Horizontal overflow | Console/page errors |
|---|---|---:|---|---|---:|
| Landing | `/` | 200 | yes | no | 0 |
| Discover | `/discover` | 200 | yes | no | 0 |
| Paid event detail | `/events/apac-stellar-builder-meetup` | 200 | yes | no | 0 |
| Checkout | `/events/apac-stellar-builder-meetup/checkout` | 200 | yes | no | 0 |
| Locked resources | `/events/apac-stellar-builder-meetup/resources` | 200 | yes | no | 0 |
| Pass library | `/passes` | 200 | yes | no | 0 |
| Pass receipt | `/passes/qpass-apac-stellar-builder-meetup-0001-0dc175` | 200 | yes | no | 0 |
| Organizer check-in | `/check-in/evt_apac_stellar_builder_meetup?token=qpass-apac-stellar-builder-meetup-0001-0dc175` | 200 | yes | no | 0 |
| Event proof | `/events/apac-stellar-builder-meetup/proof` | 200 | yes | no | 0 |
| Studio wallet gate | `/dashboard` | 200 | yes | no | 0 |
| Create event | `/dashboard/events/new` | 200 | yes | no | 0 |
| Collaborator ledger | `/dashboard/ledger` | 200 | yes | no | 0 |
| Evidence hub | `/evidence` | 200 | yes | no | 0 |

## Mobile

- Viewport: `390 x 844`

| Page | Path | HTTP | Required text | Horizontal overflow | Console/page errors |
|---|---|---:|---|---|---:|
| Landing | `/` | 200 | yes | no | 0 |
| Discover | `/discover` | 200 | yes | no | 0 |
| Paid event detail | `/events/apac-stellar-builder-meetup` | 200 | yes | no | 0 |
| Checkout | `/events/apac-stellar-builder-meetup/checkout` | 200 | yes | no | 0 |
| Locked resources | `/events/apac-stellar-builder-meetup/resources` | 200 | yes | no | 0 |
| Pass library | `/passes` | 200 | yes | no | 0 |
| Pass receipt | `/passes/qpass-apac-stellar-builder-meetup-0001-0dc175` | 200 | yes | no | 0 |
| Organizer check-in | `/check-in/evt_apac_stellar_builder_meetup?token=qpass-apac-stellar-builder-meetup-0001-0dc175` | 200 | yes | no | 0 |
| Event proof | `/events/apac-stellar-builder-meetup/proof` | 200 | yes | no | 0 |
| Studio wallet gate | `/dashboard` | 200 | yes | no | 0 |
| Create event | `/dashboard/events/new` | 200 | yes | no | 0 |
| Collaborator ledger | `/dashboard/ledger` | 200 | yes | no | 0 |
| Evidence hub | `/evidence` | 200 | yes | no | 0 |

## Readiness Signals

- Landing renders the Figma-aligned Quorum value proposition, feature section,
  FAQ, and primary start CTA.
- Discover renders paid and free seeded events.
- Event detail shows checkout, capacity, and collaborator split information.
- Checkout renders review, Freighter approval, split, and post-confirmation
  access context.
- Resource page renders a locked state without a pass session.
- Pass library and pass receipt render from a temporary pass-owner session.
- Organizer check-in renders from a temporary organizer session with a checked-in
  local-proof pass.
- Studio renders a focused wallet gate before a wallet session exists, followed
  by the hosting, collaborating, and attending work it will organize.
- Create event, collaborator ledger, event proof, and global evidence routes are
  included in the wide-desktop, tablet, and mobile matrix.

## Remaining Boundary

The contracts and public Vercel app now exist, but this historical local run does
not verify the current production schema, hosted indexer, current-origin testnet
evidence, or the final recovery branch.
