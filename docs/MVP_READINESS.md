...
| Mock live transaction flow persists only after success | Verified local | `npm run live:flow:smoke` covers checkout action preparation from DB state, RPC preflight, mock Freighter signing options, mock submission/finality, decoded token ID persistence after success, and no persistence after failed finality. |
