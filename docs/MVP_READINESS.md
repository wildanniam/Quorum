...
| Mock live transaction flow persists only after success | Verified local | `npm run live:flow:smoke` covers publish, checkout, check-in, and withdraw action preparation from DB state, RPC preflight, mock Freighter signing options, mock submission/finality, decoded token ID and withdraw amount persistence after success, and no persistence after failed finality. |
