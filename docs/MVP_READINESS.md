...
| Signed transaction submission boundary polls finality and decodes return values | Verified local | `npm run live:submission:smoke` covers signed transaction submission, `getTransaction` finality polling, purchase token ID decoding, withdraw amount decoding, RPC rejection, failed finality, timeout handling, and missing/wrong return values with a mock RPC boundary. |
| Browser live flow can preflight, sign, and submit signed XDR | Verified local | `npm run live:browser-flow:smoke` covers the browser helper calling the preflight route, passing Freighter signer options, posting signed XDR to the submit route, and surfacing preflight/submit errors without opening Freighter. |
