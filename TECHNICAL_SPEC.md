...
Phase 5 spike decision:

- Use the workspace layout generated by `stellar contract init`.
- The local CLI 26 scaffold currently uses `soroban-sdk = "25"`; keep the generated SDK line while tests/build pass.
- Implement a custom minimal `QuorumPassNFT` for the MVP instead of importing OpenZeppelin NFT modules, because Quorum needs non-transferable passes, `QuorumCore`-only minting, and event-owner lookup as first-class constraints.
- Keep OpenZeppelin Stellar Contracts as a post-MVP hardening/reference option.
