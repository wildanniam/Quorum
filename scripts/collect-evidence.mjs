...
const reuseBrowserQa = process.argv.includes("--reuse-browser-qa");
const browserQaCheck = reuseBrowserQa
  ? {
      label: "Browser QA",
      command: "npm",
...
const checks = [
  ...
  browserQaCheck,
  {
    label: "Browser QA provenance smoke",
    command: "npm",
...
const browserQa = verificationResults.find(
  (result) => result.label === "Browser QA",
);
...
const browserQaJson = browserQa ? extractJson(browserQa.output) : null;
...
`,
  ...
  .join("\n");
const browserQaEvidenceMode = reuseBrowserQa
  ? "reused-unchanged-inputs"
  : "fresh-playwright-run";
const browserQaProvenanceLines = reuseBrowserQa
  ? `- Browser evidence commit: \`${browserQaJson?.browserEvidenceCommit ?? "unavailable"}\`
- Browser source commit: \`${browserQaJson?.browserSourceCommit ?? "unavailable"}\`
- Current input fingerprint: \`${browserQaJson?.currentInputFingerprint ?? "unavailable"}\`
- Changed browser input files: \`${browserQaJson?.changedInputFiles?.length ?? "unknown"}\``
  : "- Browser source and result were regenerated by `npm run browser:qa`.";
...
- Mode: \`${browserQaEvidenceMode}\`
${browserQaProvenanceLines}
