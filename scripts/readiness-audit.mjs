...
const requiredFiles = [
  ...
  "scripts/browser-qa-provenance-smoke.mjs",
...
function checkEvidence() {
  ...
  const reusedBrowserQa = evidence.includes(
    "- Mode: `reused-unchanged-inputs`",
  );
  const freshBrowserQa = evidence.includes(
    "- Mode: `fresh-playwright-run`",
  );
  ...
  if (!reusedBrowserQa && !freshBrowserQa) {
    fail("DEMO_EVIDENCE does not identify how browser QA was verified.");
  }

  if (reusedBrowserQa) {
    try {
      const browserProvenance = runJson("node", [
        "scripts/browser-qa-provenance.mjs",
      ]);

      if (!browserProvenance.ok) {
        fail("Browser QA provenance did not validate unchanged inputs.");
      }

      for (const value of [
        browserProvenance.browserEvidenceCommit,
        browserProvenance.browserSourceCommit,
        browserProvenance.currentInputFingerprint,
      ]) {
        if (!value || !evidence.includes(value)) {
          fail(`DEMO_EVIDENCE is missing browser provenance value: ${value}`);
        }
      }
    } catch (error) {
      fail(
        `Browser QA provenance could not be verified: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
