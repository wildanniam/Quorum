...
function checkEvidence() {
  ...
  if (!evidence.includes("- Payment asset configured: `true`")) {
    fail("DEMO_EVIDENCE does not record configured payment asset readiness.");
  }

  if (!evidence.includes("QUORUM_LIVE_SIGNING_APPROVED=I_APPROVE_TESTNET_SIGNING")) {
    fail("DEMO_EVIDENCE does not document the explicit signing approval gate.");
  }

  if (!evidence.includes("CBIELTK6YBZJU5UP2WWQEUCYKLPU6AUNZ2BQ4WWFEIE3USCIHMXQDAMA")) {
    fail("DEMO_EVIDENCE does not document the configured testnet USDC contract.");
  }
